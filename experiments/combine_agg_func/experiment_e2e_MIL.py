...
def launch_experiment(
    ...
    afterNN = torch.nn.Sequential(
        torch.nn.Linear(2*256,256), # double the size due to concetanation of outputs of aggregation functions
        torch.nn.ReLU(),
    ...
    # define used aggregation functions
    agg_1 = torch.mean 
    agg_2 = aggregation_max
    ...
    model = BERTPlusMILModel(
        afterNN=afterNN,
        aggregation_func = agg_1,
        secondary_agg_func = agg_2,
        num_labels=2,
...
launch_experiment(
    ...
    name = "E2E 40k set with MEAN+MAX with grad accumulation, grad_acc 10",
    learning_rate = 2e-5,
    n_epochs=20,
    batch_size=4)
