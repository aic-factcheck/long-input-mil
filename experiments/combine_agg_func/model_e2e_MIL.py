...
class BERTPlusMILModel(pl.LightningModule):
    def __init__(
        ...
        self.secondary_agg_func = secondary_agg_func
        self.prepNN = prepNN
    ...
    def forward(self, ids_bert, attention_mask, token_type_ids, batch_ids):
        # first use BERT to create embeddings
        ...
        output = torch.empty((len(unique), 2*len(NN_out[0])), device = device)
        
        for i in range(len(unique)):
            agg_1 = self.aggregation_func(NN_out[ids == i], dim = 0).squeeze()
            agg_2 = self.secondary_agg_func(NN_out[ids == i], dim = 0)
            tmp_output = torch.cat((agg_1, agg_2), dim=0) # merge outputs of 
            output[i] = tmp_output
